...
scratch_repo="localhost:5000/portier-nginx-auth"
upload_repo="portier/nginx-auth"
...
for build_arch in "${build_archs[@]}"; do
  ...
  tar -cC ./release/portier-nginx-auth-v${version}-linux-${PKGARCH} portier-nginx-auth \
    | docker import --change 'ENTRYPOINT ["/portier-nginx-auth"]' - "${manifest}"
